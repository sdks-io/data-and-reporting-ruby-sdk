...
| `is_super_admin` | `TrueClass \| FalseClass` | Optional | Whether the user is a super administrator<br><br>**Default**: `false` |
...
| `display_week` | `TrueClass \| FalseClass` | Optional | Preferred Display Week on Fuel Prices configuration for the logged in User.<br><br>**Default**: `true` |
...
| `has_api_access` | `TrueClass \| FalseClass` | Optional | True/False<br>True, if user has access to the requested API. This is validated based on the role Vs API access matrix and client Vs API access matrix.<br><br>**Default**: `true` |
